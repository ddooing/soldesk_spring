...
<c:import url="/WEB-INF/views/include/admin_header.jsp" />
											...
											<tr style="vertical-align: middle;">
												...
												<c:choose>
											        <c:when test="${bannerPaylist.state == 1 || bannerPaylist.state == 2}">
											            <td style="width: 200px;">${bannerPaylist.payment}</td>
											        </c:when>
											        
											        <c:when test="${bannerPaylist.state == 3}">
											            <td style="width: 200px; color: #dc3545;">- ${bannerPaylist.payment}</td>
											        </c:when>
											    </c:choose>
												...
												<c:choose>
											        <c:when test="${bannerPaylist.banner_type == 1}">
											            <td style="width: 200px;">메인배너</td>
											        </c:when>
											        
											        <c:when test="${bannerPaylist.banner_type == 2}">
											            <td style="width: 200px; color: #dc3545;">서브배너</td>
											        </c:when>
											    </c:choose>
												...
												<td style="width: 150px;">
													<c:choose>
												        <c:when test="${bannerPaylist.state == 1 || bannerPaylist.state == 2}">
												            <button data-apply_form_id="${bannerPaylist.banner_apply_form_id}" class="btn btn-danger"style="float: right; margin-right: 30px;
													 margin-top: 20px; margin-bottom: 20px;" onclick="showConfrimCancle(this.getAttribute('data-apply_form_id'))">취소</button>
	...
	<script>
		function showConfrimCancle(banner_form_id) {
			...
			Swal.fire({
	            ...
	            if (result.isConfirmed) {
	            	 window.location.href = '${root}/adminPayment/bannerPayment_cancel?banner_form_id='+banner_form_id;
