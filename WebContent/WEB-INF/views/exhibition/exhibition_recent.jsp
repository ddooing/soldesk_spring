	...
	<section class="vh-60">
		...
		<div style="margin-top: 30px; text-align: center; margin-right: 850px;">
			...
			<button id="recentbtn" type="button" onclick="window.location.href='${root}/exhibition/exhibition_recent'"
				style="color: black; font-size: 25px; text-decoration: none; border: none; background-color: transparent;  margin-left: 15px;">최신</button>
		    ...
		    <div style="margin-top: 50px; display: flex; margin-left:35px; flex-wrap: wrap; justify-content: start;">
		        <c:forEach items="${getRecentExhibitionInfo}" var="recentexhibition">
		            <div style="padding: 10px; width: calc(25%-30px); box-sizing: border-box;">
		                ...
		                <p style="font-size: 20px; width:255px; text-align: center; margin-top:30px; word-wrap: break-word;">${recentexhibition.title }</p>
		            </div>
		        </c:forEach>
		    </div>
		</div>
		...
		<div class="d-none d-md-block" style="margin-top:50px;">
				<ul class="pagination justify-content-center">
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item">
								<a href="${root }/exhibition/exhibition_recent?page=${pageBean.prevPage}" class="page-link">이전</a>
							</li>
						</c:otherwise>
					</c:choose>
						...
						<c:choose>
							<c:when test="${idx == pageBean.currentPage }">
								<li class="page-item active" >
									<a href="${root }/exhibition/exhibition_recent?page=${idx}" class="page-link">
										${idx }
									</a>
								</li>
							</c:when>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item">
								<a href="${root }/exhibition/exhibition_recent?page=${pageBean.nextPage}" class="page-link">다음</a>
							</li>
						</c:otherwise>
					</c:choose>
				</ul>
